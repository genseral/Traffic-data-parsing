...
re_event_type = r'(VST|ST)'  # Type of event message.
re_time = r'\d\d\d\d-\d\d-\d\d.\d\d:\d\d:\d\d.\d\d\d'  # Definition of datetime.
re_event_trigger = r'TTG=(\d)'  # Unique identifier for what triggered the event.
re_intersection = r'\d\d\d(?=\.)'  # Unique identifier of intersection.
re_device = r'(d|sg)'  # Unique identifier for detector or traffic signal.
re_id = r'[sg|d].([0-9]+)'  # Unique device identifier.
